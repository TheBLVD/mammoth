...
extension PostCardModel {
    ...
    public func likeTap() -> Void {
        guard case .mastodon(let status) = data, (status.reblog?.favourited ?? status.favourited) == false else { return }
        (status.reblog ?? status).likeTap()
    }
    
    public func unlikeTap() -> Void {
        guard case .mastodon(let status) = data, (status.reblog?.favourited ?? status.favourited) == true else { return }
