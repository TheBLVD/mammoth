    ...
    internal func saveToDisk(items: [NewsFeedListItem]?,
        ...
        savingQueue.async {
            ...
            if let scrollPositionCardIndex {
                // Keep the bookmarked card, 8 younger cards, and 10 older cards
                var cardsSubset = items?[max(scrollPositionCardIndex - 8, 0)...min(scrollPositionCardIndex + 10, (items?.count ?? 1) - 1)]
